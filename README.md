...
[快速开始](#快速开始) · [成熟玩法](#成熟玩法) · [完整游戏示例](#随仓库提供的完整游戏示例) · [系统结构](#系统结构) · [配置](#配置) · [质量与安全](#质量与安全) · [许可证](#许可证与第三方边界)
...
仓库直接包含当前完成版游戏“虫虫攀枝”，位于 [`examples/bug-climb/`](examples/bug-climb/)。它不是依赖本机数据库的临时构建，而是一份可独立运行的完整静态交付包，包含游戏代码、AI 生成位图、角色素材、资源溯源、设计记录和桌面/手机端质量截图。

安装依赖后可直接启动：

```bash
npm run demo:bug-climb
```

打开 `http://127.0.0.1:4322/` 即可试玩。当前纳入仓库的是 v13：瓢虫身体位图不再包含固定腿，六条动态腿分别连接胸部前、中、后三组关节，并保留上下左右移动、头部拾取露珠和露珠爆开反馈。
...
每个游戏必须生成并实际使用可追溯的 AI 位图。构建至少需要 `assets/cover.png` 和 `assets/background.png`，并在 `_studio/DYNAMIC_ART.json` 中保存模型、用途、文件路径和完整提示词。

平台拒绝以下产物：

- 未配置 OpenAI API Key，无法生成所需位图
- 缺少封面或局内背景
- 使用 SVG 文件、内联 `<svg>`、SVG MIME 或 `.svg` 资源引用
- 只有 Canvas、CSS 几何图形或 Emoji，没有实际加载 AI 局内背景
- 位图文件与生成记录不一致

密钥可通过工作台的“模型”设置、环境变量或本机密钥文件提供；真实密钥不会写入仓库、数据库或游戏产物。不配置密钥时仍可浏览项目和历史版本，但新构建会明确中断，不会退回占位图。
...
| `OPENAI_API_KEY` | 未设置 | 构建必填；也可改用工作台会话设置或本机密钥文件 |
| `OPENAI_API_KEY_FILE` | `./data/secrets/openai-api-key.txt` | 构建必填来源之一；仅保存在本机且不进入 Git |
