...
const modes = new Set(["collector", "arena", "popup"]);
const entries = process.argv.slice(2).map((entry) => {
  const separator = entry.indexOf("=");
  if (separator < 1) throw new Error(`参数格式应为 collector|arena|popup=artifact-root：${entry}`);
...
for (const [mode, root] of entries) {
  if (!modes.has(mode)) throw new Error(`未知 3D 模式：${mode}`);
