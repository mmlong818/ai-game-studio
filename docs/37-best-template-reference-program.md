...
自 2026-09-03 起，每款官方游戏（含模板游戏、固定游戏与 3D 示范游戏）都以 `src/shared/official-games/<id>.ts` 为唯一登记来源；玩法模板、验收场景、运行时定义、服务端枚举、固定游戏注册与大厅顺序全部由它派生。新增或调整一款游戏按 `docs/55-adding-an-official-game.md` 操作，不再手写各处映射表。

官方游戏与改造模板分别验收：游戏可以先作为可玩的官方短局上线；只有登记中的核心循环、输入、反馈、机制研究映射和实际产物一致时，才允许作为改造模板。单款游戏修复后必须同时更新固定产物、登记文件、参照文档与回归测试，不能只改正在服务的版本目录。
...
| 11 | 星灵巡格 | `region-logic` | 已完成并发布 v19，见 `49-region-logic-best-template-reference.md` | 20 个原创唯一解关卡全部可直接选择，一星到二星五章、8 套不同双星答案、无猜测推理轨迹、解释型提示、局内撤销/重做及重新打开空盘均已验证；稳定网址已指向 `e6250710-cc9f-4aa8-a220-e2339f25f99a` |
...
- 长线关卡模板至少提供 20 关或等价的 20 个可识别进度目标；明确定位为单局制的街机或短程游戏，改为验证一局内至少三个可辨认阶段、稳定难度递进和可重复结算，不虚构关卡数量。
