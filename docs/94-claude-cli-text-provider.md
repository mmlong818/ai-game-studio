...
- 每次调用都是独立的对话：纯文本请求 `--max-turns 1`；带 `--json-schema` 的结构化请求 `--max-turns 3`，因为 CLI 用内置 `StructuredOutput` 工具承载结构化输出并按 schema 校验，模型格式失误时需要额外的重写轮（详见 [98](98-claude-cli-structured-output-turns.md)）。`--tools ""`（不开任何工具）、`--strict-mcp-config`（不加载 MCP）、`--setting-sources ""`（不加载用户与项目设置）、`--no-session-persistence`（不落会话文件）。系统提示通过临时文件传入，结束后删除；用户提示走标准输入。结构化输出使用 `--json-schema`；当命令行放不下 schema 时改由系统提示承载约束，输出仍经 zod 校验。
