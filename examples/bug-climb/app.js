...
function showIdle(){overlay.classList.remove('hidden');ui.restart.hidden=true;modal.innerHTML='<span class="eyebrow">树冠远足队</span><h1>虫虫攀枝</h1><div class="goal">贴着树瘤险险绕过，头部碰到露珠即可吸收；能量充足时加速冲破琥珀树脂，一路爬向树冠。</div><div class="carList">'+carsHTML()+'</div><div class="settings"><button class="toggle" data-set="high">高对比</button><button class="toggle" data-set="colorblind">色觉辅助</button><button class="toggle" data-set="noFlash">柔和动效</button></div><p class="mini">使用上下左右在树皮上爬动。水滴碰到头部会爆开并转成露珠能量；树瘤要绕开，琥珀树脂需要按住冲刺撞碎。</p><button class="primary" id="start">开始爬树</button>';
}
...
function collide(e){if(e.res)return;e.res=true;var colors=palette();if(e.kind==='chip'){nitro=Math.min(nitroMax,nitro+25);score+=80*mult;burstAt(e.lane,e.d,colors.cyan,20,1.25);nearPulse=Math.max(nearPulse,.55);kickText('露珠爆开 +25',colors.cyan,.58);tone(620+nitro*2,.13,'sine',.032);tone(930,.16,'triangle',.017,.05);showToast('头部吸收露珠 +25',520);return}if(e.kind==='shard'){score+=250*mult;burstAt(e.lane,e.d,colors.yellow,15,1);kickText('金色种子 +250',colors.yellow,.62);tone(784,.18,'triangle',.032);tone(1047,.2,'sine',.019,.06);showToast('发现一颗金色种子',620);return}if(e.kind==='red'&&boosting&&nitro>0){score+=300*mult;shake=reducedMotion?0:.68;nearPulse=1;burstAt(e.lane,e.d,colors.red,28,1.75);kickText('树脂碎开 +300',colors.white,.86);flash();tone(220,.16,'triangle',.04);tone(330,.18,'sine',.025,.05);showToast('冲破树脂 · 连续保持',720);if(navigator.vibrate)navigator.vibrate([35,22,65]);return}if(invuln>0)return;hp--;collisions++;mult=1;invuln=1.15;damagePulse=1;shake=reducedMotion?0:.55;burstAt(e.lane,e.d,colors.red,18,1.25);kickText('撞到啦 · 连续清零',colors.red,.9);tone(174,.26,'triangle',.036);tone(131,.3,'sine',.022,.06);showToast(e.kind==='red'?'露珠不足，树脂挡住了去路':'撞到树瘤！请及时换树纹',900);if(navigator.vibrate)navigator.vibrate(90);if(hp<=0)lose('体力降到0。树瘤要绕开，琥珀树脂需要按住冲刺撞碎。');}
...
function update(dt){
  ...
  for(var i=entities.length-1;i>=0;i--){
    ...
    if(!e.res&&entityPassedPlayer(e)&&(e.kind==='normal'||e.kind==='red')){
      ...
      if(diff<1.2){nearMisses++;mult=Math.min(5,mult+1);nitro=Math.min(nitroMax,nitro+12);score+=120*mult;nearPulse=1;shake=reducedMotion?0:.2;burstAt(e.lane,20,palette().cyan,14,1.1);kickText('贴边险避 · 露珠 +12 · ×'+mult,palette().cyan,.82);flash();tone(698,.1,'triangle',.027);tone(988,.12,'sine',.019,.07);showToast('险险绕过 · 连续 ×'+mult,650)}
    }
...
function drawLaneWarning(e,colors){var nearD=Math.max(5,e.d-18),farD=Math.min(ROAD.far,e.d+125),laneCenter=(e.lane-1)*2/3,leftN=laneCenter-.275,rightN=laneCenter+.275,tint=e.kind==='red'?colors.red:colors.yellow;quad(roadPoint(leftN,farD),roadPoint(rightN,farD),roadPoint(rightN,nearD),roadPoint(leftN,nearD),e.kind==='red'?'rgba(246,173,61,.18)':'rgba(255,231,126,.14)');ctx.strokeStyle=tint;ctx.lineWidth=2+roadGeometry(e.d).depth*3;[36,72,108].forEach(function(offset){var d=Math.min(ROAD.far,e.d+offset),a=roadPoint(laneCenter-.12,d),b=roadPoint(laneCenter,d-18),c=roadPoint(laneCenter+.12,d);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.lineTo(c.x,c.y);ctx.stroke()})}
...
function drawEntity(e){if(e.d>ROAD.far+10||e.d<-80||e.res&&(e.kind==='chip'||e.kind==='shard'))return;var colors=palette(),q=projection(e.lane,e.d),w=entityWidth(e);if(e.warn&&!e.res&&(e.kind==='normal'||e.kind==='red')){ctx.save();ctx.translate(450,0);ctx.scale(sceneScaleX,1);ctx.translate(-450,0);drawLaneWarning(e,colors);ctx.restore()}ctx.save();ctx.translate(mobileX(q.x),q.y);if(e.kind==='chip'||e.kind==='shard')drawCollectible(e,q,w,colors);else drawBarrier(e,q,w,colors);ctx.restore()}
