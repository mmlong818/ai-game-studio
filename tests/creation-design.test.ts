...
test("确认的结构化方案原样复用，新游戏不匹配固定模板、不调用模型", async () => {
  ...
  assert.deepEqual(spec.designProfile, profile);
...
test("花园翻牌真实动作进入分析和可执行教学，不使用待细化或词库模板动作", async () => {
  const garden = {
    ...profile,
  ...
  const plan = createOnboardingRuntimePlan(contract)!;
  assert.deepEqual(plan.steps.map(step => step.instruction), garden.onboarding);
  assert.doesNotMatch(JSON.stringify({ analysis: result.analysis, plan }), /待细化|主要操作待/);
  ...
  assert.deepEqual(createOnboardingRuntimePlan(rebuilt)!.steps.map(step => step.instruction), garden.onboarding);
});
