...
const zhCN = {
  ...
  "models.trigger": "模型",
  "models.title": "模型与密钥",
  "models.description": "文字模型负责规划和代码；只有确认方案采用位图时才需要 OpenAI 图片 Key。程序绘制或复用资源不会因缺少图片 Key 中断。",
  ...
  "models.unconfigured": "尚未配置 OpenAI Key · 位图路线不可用",
  "models.session": "已配置 · 当前服务器会话",
  "models.environment": "已配置 · 服务器环境变量",
  "models.file": "已配置 · 本机密钥文件",
  "models.save": "保存密钥",
  "models.clear": "清除会话密钥",
  "models.close": "关闭模型设置",
  ...
  "models.descriptionDetail": "文字模型用于规划和代码；只有确认方案采用位图时才需要图片 Key。程序绘制路线可使用已配置的本地文字提供方。",
  "models.loading": "正在读取模型设置…",
  "models.source.environment": "环境变量", "models.source.file": "本机文件", "models.source.session": "当前会话", "models.source.server": "服务端",
  "models.status.claudeWithImage": "文字请求已配置为 Claude CLI（首次调用时验证）；OpenAI 图片 Key 已配置（{source}）",
  "models.status.claudeNoImage": "文字请求已配置为 Claude CLI（首次调用时验证）；位图路线尚未配置 OpenAI 图片 Key",
  "models.status.openai": "OpenAI 文字与图片 Key 已配置（{source}；首次制作时验证连接）",
  "models.status.none": "尚未配置文字提供方；位图路线也未配置 OpenAI 图片 Key",
  "models.keyConfiguredPlaceholder": "已配置，更换时填写新 Key",
  "models.connectionHint": "保存设置不会开始制作游戏；实际制作会使用模型服务额度。",
  "models.details": "模型选择与连接详情", "models.fixed": "{status}。模型由服务端配置，不能在此更换。",
  "models.textModel": "文本模型：{model}", "models.imageModel": "图像模型：{model}",
  "models.validationNote": "保存只检查 Key 格式，不会测试连接，也不会自动发起游戏制作。",
  "models.saved": "OpenAI 图片 Key 已保存；文字模型使用 {textModel}，位图路线使用 {imageModel}。",
  "models.actionFailed": "连接或保存设置没有完成。请按上面的下一步处理；不会修改已有设置。",
  "home.hero": "把想法变成\n一个能玩的世界。",
  ...
  "nav.knowledge": "策划库", "brand.tagline": "游戏创作工作室 · V{version}",
  "library.coverAlt": "{title} 游戏封面", "library.loadFailed": "游戏大厅读取失败。",
  "preview.detail": "自己的游戏，随时试玩。不需要先公开发布。", "preview.back": "返回作品空间",
  "preview.loading": "正在打开最近成功的版本…", "preview.unavailable": "还没有通过检查的可玩版本。返回作品空间查看制作进度。",
  "preview.loadFailed": "暂时无法读取可玩版本。你的游戏不会被重新制作，请稍后刷新。", "preview.frameTitle": "{title}试玩",
  "projects.loadFailed": "项目加载失败。", "projects.detailLoadFailed": "项目详情加载失败。",
  "document.studio": "{title} · 制作台", "document.home": "造界 · AI 游戏工坊", "document.create": "游戏创作 · 造界",
  "studio.previewOldFailed": "新版未完成，仍可试玩上一个成功版本。", "studio.previewOldBuilding": "新版正在制作，当前仍可试玩上一个成功版本。", "studio.runtime.building": "制作中", "studio.runtime.playable": "可试玩", "studio.runtime.waiting": "等待中", "studio.previewTagline": "直接试玩 · 继续游玩 · 分享",
  "cover.unavailableLabel": "{title}暂不可用", "cover.unavailable": "封面暂不可用", "cover.continue": "可继续查看作品",
} as const;
...
const zhTW: MessageTable = {
  ...
  "models.trigger": "模型", "models.title": "模型與金鑰", "models.description": "文字模型負責規劃和程式碼；只有確認方案採用點陣圖時才需要 OpenAI 圖片金鑰。程序繪製或重用資源不會因缺少圖片金鑰中斷。", "models.available": "可用模型", "models.keyLabel": "OpenAI API Key", "models.security": "金鑰只傳送到本機服務端，不寫入瀏覽器、資料庫、專案或遊戲產物；服務重啟後會清除。正式環境可使用 OPENAI_API_KEY。", "models.unconfigured": "尚未設定 OpenAI 金鑰 · 點陣圖路線不可用", "models.session": "已設定 · 目前服務器工作階段", "models.environment": "已設定 · 服務器環境變數", "models.file": "已設定 · 本機金鑰檔案", "models.save": "儲存金鑰", "models.clear": "清除工作階段金鑰", "models.close": "關閉模型設定", "models.loadFailed": "無法讀取模型設定。", "models.saveFailed": "無法儲存金鑰。", "models.clearFailed": "無法清除金鑰。",
...
const en: MessageTable = {
  ...
  "models.trigger": "Models", "models.title": "Models & API Key", "models.description": "The text model handles planning and code. An OpenAI image key is needed only when the confirmed plan uses bitmaps; procedural or reused assets do not require one.", "models.available": "Available models", "models.keyLabel": "OpenAI API Key", "models.security": "The key is sent only to the local server. It is not written to the browser, database, project, or game output, and is cleared on restart. Use OPENAI_API_KEY in production.", "models.unconfigured": "OpenAI key not configured · bitmap route unavailable", "models.session": "Configured · current server session", "models.environment": "Configured · server environment", "models.file": "Configured · local key file", "models.save": "Save key", "models.clear": "Clear session key", "models.close": "Close model settings", "models.loadFailed": "Could not load model settings.", "models.saveFailed": "Could not save the key.", "models.clearFailed": "Could not clear the key.",
...
const ja: MessageTable = {
  ...
  "models.trigger": "モデル", "models.title": "モデルと API キー", "models.description": "テキストモデルは設計とコード生成に使います。OpenAI の画像キーが必要なのは、確定した案でビットマップを使う場合だけです。プログラム描画や既存素材の再利用には不要です。", "models.available": "利用可能なモデル", "models.keyLabel": "OpenAI API Key", "models.security": "キーはローカルサーバーにのみ送信され、ブラウザー、DB、プロジェクト、ゲーム成果物には保存されません。サーバー再起動時に消去されます。本番環境では OPENAI_API_KEY を使用できます。", "models.unconfigured": "OpenAI キー未設定 · ビットマップ経路は利用不可", "models.session": "設定済み · 現在のサーバーセッション", "models.environment": "設定済み · サーバー環境変数", "models.file": "設定済み · ローカルキーファイル", "models.save": "キーを保存", "models.clear": "セッションキーを消去", "models.close": "モデル設定を閉じる", "models.loadFailed": "モデル設定を読み込めませんでした。", "models.saveFailed": "キーを保存できませんでした。", "models.clearFailed": "キーを消去できませんでした。",
...
Object.assign(zhTW, {
  "models.descriptionDetail": "文字模型用於規劃和程式碼；只有確認方案採用點陣圖時才需要圖片金鑰。程序繪製可使用已設定的本機文字提供方。", "models.loading": "正在讀取模型設定…", "models.source.environment": "環境變數", "models.source.file": "本機檔案", "models.source.session": "目前工作階段", "models.source.server": "服務端",
  "models.status.claudeWithImage": "文字請求已設定為 Claude CLI（首次呼叫時驗證）；OpenAI 圖片金鑰已設定（{source}）", "models.status.claudeNoImage": "文字請求已設定為 Claude CLI（首次呼叫時驗證）；點陣圖路線尚未設定 OpenAI 圖片金鑰", "models.status.openai": "OpenAI 文字與圖片金鑰已設定（{source}；首次製作時驗證連線）", "models.status.none": "尚未設定文字提供方；點陣圖路線也未設定 OpenAI 圖片金鑰", "models.keyConfiguredPlaceholder": "已設定，更換時填寫新金鑰", "models.connectionHint": "儲存設定不會開始製作遊戲；實際製作會使用模型服務額度。", "models.details": "模型選擇與連線詳情", "models.fixed": "{status}。模型由服務端設定，不能在此更換。", "models.textModel": "文字模型：{model}", "models.imageModel": "圖片模型：{model}", "models.validationNote": "儲存只檢查金鑰格式，不會測試連線，也不會自動開始遊戲製作。", "models.saved": "OpenAI 圖片金鑰已儲存；文字模型使用 {textModel}，點陣圖路線使用 {imageModel}。", "models.actionFailed": "連線或儲存設定未完成。請依照上方步驟處理；現有設定不會變更。",
  "nav.knowledge": "策劃庫", "brand.tagline": "遊戲創作工作室 · V{version}", "library.coverAlt": "{title} 遊戲封面", "library.loadFailed": "無法讀取遊戲大廳。", "preview.detail": "自己的遊戲，隨時試玩。不需要先公開發佈。", "preview.back": "返回作品空間", "preview.loading": "正在開啟最近成功的版本…", "preview.unavailable": "還沒有通過檢查的可玩版本。請返回作品空間查看製作進度。", "preview.loadFailed": "暫時無法讀取可玩版本。遊戲不會重新製作，請稍後重新整理。", "preview.frameTitle": "試玩 {title}", "projects.loadFailed": "無法載入專案。", "projects.detailLoadFailed": "無法載入專案詳情。", "document.studio": "{title} · 製作台", "document.home": "造界 · AI 遊戲工坊", "document.create": "遊戲創作 · 造界",
  "studio.previewOldFailed": "新版未完成，仍可試玩上一個成功版本。", "studio.previewOldBuilding": "新版正在製作，目前仍可試玩上一個成功版本。", "studio.runtime.building": "製作中", "studio.runtime.playable": "可試玩", "studio.runtime.waiting": "等待中", "studio.previewTagline": "直接試玩 · 繼續遊玩 · 分享",
  "cover.unavailableLabel": "{title}暫不可用", "cover.unavailable": "封面暫不可用", "cover.continue": "可繼續查看作品",
});
Object.assign(en, {
  "models.descriptionDetail": "The text model handles planning and code. An image key is needed only when the confirmed plan uses bitmaps; procedural routes can use a configured local text provider.", "models.loading": "Loading model settings…", "models.source.environment": "environment", "models.source.file": "local file", "models.source.session": "current session", "models.source.server": "server",
  "models.status.claudeWithImage": "Text requests use Claude CLI (verified on first use); the OpenAI image key is configured ({source})", "models.status.claudeNoImage": "Text requests use Claude CLI (verified on first use); the bitmap route has no OpenAI image key", "models.status.openai": "OpenAI text and image key configured ({source}; connection verified on first build)", "models.status.none": "No text provider is configured, and the bitmap route has no OpenAI image key", "models.keyConfiguredPlaceholder": "Configured; enter a new key to replace it", "models.connectionHint": "Saving settings does not start a build; building uses model-service quota.", "models.details": "Model and connection details", "models.fixed": "{status}. Models are configured by the server and cannot be changed here.", "models.textModel": "Text model: {model}", "models.imageModel": "Image model: {model}", "models.validationNote": "Saving checks only the key format. It does not test the connection or start a build.", "models.saved": "OpenAI image key saved. Text uses {textModel}; bitmap routes use {imageModel}.", "models.actionFailed": "The connection or settings update did not complete. Follow the step above; existing settings remain unchanged.",
  "nav.knowledge": "Design Library", "brand.tagline": "GAME CREATION STUDIO · V{version}", "library.coverAlt": "Cover art for {title}", "library.loadFailed": "Could not load the game library.", "preview.detail": "Play your own game any time, without publishing it first.", "preview.back": "Back to project", "preview.loading": "Opening the latest successful version…", "preview.unavailable": "There is no playable version that has passed checks yet. Return to the project to see build progress.", "preview.loadFailed": "The playable version could not be loaded. Your game will not be rebuilt; refresh later.", "preview.frameTitle": "Play {title}", "projects.loadFailed": "Could not load projects.", "projects.detailLoadFailed": "Could not load project details.", "document.studio": "{title} · Studio", "document.home": "Forge · AI Game Studio", "document.create": "Game Creation · Forge",
  "studio.previewOldFailed": "The new version did not finish; the previous successful version is still playable.", "studio.previewOldBuilding": "The new version is building; the previous successful version remains playable.", "studio.runtime.building": "Building", "studio.runtime.playable": "Playable", "studio.runtime.waiting": "Waiting", "studio.previewTagline": "Play directly · Continue · Share",
  "cover.unavailableLabel": "{title} unavailable", "cover.unavailable": "Cover unavailable", "cover.continue": "You can still view this project",
});
Object.assign(ja, {
  "models.descriptionDetail": "テキストモデルは設計とコード生成に使います。画像キーが必要なのは確定案でビットマップを使う場合だけで、プログラム描画では設定済みのローカル文字プロバイダーを利用できます。", "models.loading": "モデル設定を読み込み中…", "models.source.environment": "環境変数", "models.source.file": "ローカルファイル", "models.source.session": "現在のセッション", "models.source.server": "サーバー",
  "models.status.claudeWithImage": "テキストは Claude CLI を使用します（初回利用時に確認）。OpenAI 画像キーは設定済みです（{source}）", "models.status.claudeNoImage": "テキストは Claude CLI を使用します（初回利用時に確認）。ビットマップ経路の OpenAI 画像キーは未設定です", "models.status.openai": "OpenAI のテキスト・画像キーは設定済みです（{source}、初回制作時に接続確認）", "models.status.none": "テキストプロバイダーも、ビットマップ経路の OpenAI 画像キーも設定されていません", "models.keyConfiguredPlaceholder": "設定済み。変更する場合は新しいキーを入力", "models.connectionHint": "設定を保存しても制作は始まりません。実際の制作ではモデルサービスの利用枠を消費します。", "models.details": "モデルと接続の詳細", "models.fixed": "{status}。モデルはサーバー側で設定され、ここでは変更できません。", "models.textModel": "テキストモデル：{model}", "models.imageModel": "画像モデル：{model}", "models.validationNote": "保存時はキーの形式だけを確認します。接続テストやゲーム制作は開始しません。", "models.saved": "OpenAI 画像キーを保存しました。テキストは {textModel}、ビットマップ経路は {imageModel} を使用します。", "models.actionFailed": "接続または設定の保存が完了しませんでした。上の手順に従ってください。既存設定は変更されません。",
  "nav.knowledge": "企画ライブラリ", "brand.tagline": "ゲーム制作スタジオ · V{version}", "library.coverAlt": "{title} のゲームカバー", "library.loadFailed": "ゲーム一覧を読み込めませんでした。", "preview.detail": "自分のゲームをいつでも試せます。先に公開する必要はありません。", "preview.back": "作品スペースへ戻る", "preview.loading": "最新の成功バージョンを開いています…", "preview.unavailable": "検査に合格したプレイ可能版はまだありません。作品スペースで制作状況を確認してください。", "preview.loadFailed": "プレイ可能版を読み込めません。ゲームを再制作することはありません。後でもう一度更新してください。", "preview.frameTitle": "{title} を試す", "projects.loadFailed": "プロジェクトを読み込めませんでした。", "projects.detailLoadFailed": "プロジェクト詳細を読み込めませんでした。", "document.studio": "{title} · 制作台", "document.home": "造界 · AI ゲーム工房", "document.create": "ゲーム制作 · 造界",
  "studio.previewOldFailed": "新版は完成していません。直前の成功版は引き続きプレイできます。", "studio.previewOldBuilding": "新版を制作中です。直前の成功版は引き続きプレイできます。", "studio.runtime.building": "制作中", "studio.runtime.playable": "プレイ可能", "studio.runtime.waiting": "待機中", "studio.previewTagline": "すぐ遊ぶ · 続ける · 共有",
  "cover.unavailableLabel": "{title} は利用できません", "cover.unavailable": "カバーを表示できません", "cover.continue": "作品は引き続き確認できます",
});
