...
export function PrivateGamePreview({ project }: { project: ProjectDetail }) {
  ...
  return <>
    ...
    <main className="private-game-preview" id="main-content" tabIndex={-1}>
      ...
      {!current ? <p role="status">正在打开最近成功的版本…</p> : current.error ? <p role="alert">{current.error}</p> : url ? <iframe title={`${project.title}试玩`} src={url} sandbox="allow-scripts allow-same-origin" allow="autoplay; fullscreen" /> : <p role="status">还没有通过检查的可玩版本。返回作品空间查看制作进度。</p>}
    </main>
  </>;
}
