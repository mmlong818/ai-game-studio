...
const designBlueprints: Record<GameTemplate, Omit<z.infer<typeof gameDesignProfileSchema>, "difficultyCurve">> = {
  ...
  platformer: {
    genre: "横版平台跳跃",
    targetPlayer: "希望在两三分钟内体验移动、跳跃和收集节奏的玩家",
    playerFantasy: "掌握角色惯性与落点，跨过裂隙抵达高处信标",
    sessionLength: "2–5 分钟",
    coreLoop: ["读取平台与危险位置", "移动并起跳", "空中修正落点", "收集能量并抵达出口"],
    winCondition: "收集最低要求的能量并抵达终点信标",
    failCondition: "掉出场景或生命值耗尽",
    progression: ["前段学习单跳与落地", "中段组合移动平台和危险区", "末段要求连续跳跃后抵达终点"],
    gameFeel: ["落地提供轻微压缩与声音", "起跳保留短暂宽容时间", "受伤后提供短暂无敌避免连续扣血"],
    onboarding: ["首个平台没有危险并标出移动方向", "第一个收集物放在自然跳跃轨迹上"],
    accessibility: ["键盘和触控提供相同移动与跳跃", "危险区使用形状、运动和颜色共同表达"],
    productionRisks: ["低帧率下碰撞不能穿透平台", "移动端同时按方向和跳跃必须可靠"],
  },
  "space-shooter": {
    genre: "俯视街机射击",
    targetPlayer: "喜欢短局压迫、明确波次和即时击破反馈的玩家",
    playerFantasy: "驾驶一架轻型飞船在不断收紧的星环中击退敌机",
    sessionLength: "2–6 分钟",
    coreLoop: ["观察敌机航线", "移动规避", "持续发射弹体", "击破目标并管理剩余生命"],
    winCondition: "击破当前难度要求的敌机数量",
    failCondition: "生命值降为零",
    progression: ["敌机生成频率逐步提高", "后段出现更快或更耐久的目标"],
    gameFeel: ["高对比弹体同时使用位图弹头、亮色核心和短尾迹，不能淹没在星空背景中", "网页端鼠标悬停跟随，手机端按住拖动，飞船使用平滑追随避免瞬移", "击破使用粒子和短促声音", "受击产生明确闪白和生命变化"],
    onboarding: ["开局短时间只出现慢速直线敌机", "默认持续射击，让玩家先专注移动"],
    accessibility: ["鼠标、键盘和按压拖动均可移动，默认持续射击", "弹体、敌机和玩家保持形状、亮度与颜色多重区分"],
    productionRisks: ["弹体和敌机离屏后必须回收", "高生成率下保持稳定帧率"],
  },
  "polyomino-fit": {
    genre: "多格拼块填形",
  ...
  "region-logic": {
    ...
    winCondition: "每行、每列和每个区域各有一个星灵，且星灵互不相邻",
    failCondition: "失误次数耗尽",
    progression: ["棋盘依难度从 5×5、6×6 或 7×7 起步，后期最高达到 8×8", "题面按唯一解、搜索节点与推理分支筛选，后期逐步撤掉引导星和直接答案提示"],
    gameFeel: ["手动与自动标记来源清晰", "错误位置短促震荡后恢复", "完成时星灵连成庆祝轨迹"],
    onboarding: ["首局分两步解释独占与相邻禁放", "默认开启自动标记降低重复操作"],
    accessibility: ["放置、标记、撤销和提示都有文字按钮", "区域同时使用边界、纹理和色相表达"],
    productionRisks: ["每题必须经求解器验证唯一解", "镜像旋转换色题必须去重"],
  },
  generated: {
    genre: "AI 生成的实验玩法",
