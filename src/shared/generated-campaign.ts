...
export function generatedCampaignPrompt(value?: unknown, options: { confirmedRules?: readonly string[] } = {}): string {
  const plan = resolveGeneratedCampaign(value);
  if (plan.mode === "endless") return `无限玩法，没有关卡、胜利目标或最终通关；不提供关卡选择器，不实现setLevel或forceWin，不得达到分数或时间阈值后自动胜利。probe 的getState返回mode:"endless"，restart重新开始一局。至少一个当前可执行的正常游戏控件或棋盘物体声明data-game-action="click"、"drag"或"key:ArrowRight"等真实输入类型；拖拽方向可用data-game-drag="left/right/up/down"声明。真实棋盘容器必须声明data-game-board；Canvas 玩法若用按钮或元素覆盖层承载操作标记，Canvas 与覆盖层必须放在这个带data-game-board的共同父容器内，不能只把标记放在Canvas的兄弟元素上却不给父容器声明棋盘根。只给当前局面少量合法且有代表性的正常操作加标记，不要给整个棋盘的所有格子批量加标记；若初始盘面没有合法操作，先用玩法本身的洗牌或继续机制恢复可操作局面。不得为验收新增“完成核心动作”等专用按钮。按声明执行的正常操作必须让玩家可见的得分、棋盘或玩法状态发生变化，重开后仍可再次操作。不得用调试counter、milestone、mechanicsActive、计时或动画帧变化冒充核心玩法。无限不表示必须增加关卡、等级、难度递增或分数系统；收集物或挑战只按确认方案持续供给。${plan.rationale}`;
