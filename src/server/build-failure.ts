...
function firstIssue(error: Error): { path?: unknown[]; message?: string } | null {
  const issues = (error as Error & { issues?: Array<{ path?: unknown[]; message?: string }> }).issues;
  return Array.isArray(issues) && issues.length ? issues[0]! : null;
}

function schemaPath(error: Error): string {
  const issues = (error as Error & { issues?: Array<{ path?: unknown[] }> }).issues;
  return Array.isArray(issues) ? issues.slice(0, 3).map(issue => (issue.path ?? []).join(".")).filter(Boolean).join("、") : "";
}
...
export function safeFailure(stage: FailureDetail["stage"], error: unknown, extra: Partial<FailureDetail> = {}): FailureDetail {
  ...
  // 规划阶段对用户输入（idea 等）的本地校验失败：是输入不合要求，不是服务返回问题；把校验消息原样告诉用户。
  const inputIssue = stage === "planning" && source.name === "ZodError" ? firstIssue(source) : null;
  const inputValidationFailure = Boolean(inputIssue && ["idea", "content", "title"].includes(String(inputIssue.path?.[0] ?? "")));
  const invalidTextResponse = !platformSchemaFailure && !inputValidationFailure && (/不是有效的 JSON|html 字段只有|缺少 html 字段|没有返回可解析的内容|未提供输出流|模型流式输出失败|输出连接中断|输出因 .*截断/.test(message)
  ...
  const repairBudgetExhausted = /修正上限|连续 \d+ 轮生成代码|轮修正后规则审核仍有|达到 \d+ 次请求上限/.test(message);
  const code = inputValidationFailure ? "INVALID_INPUT" : platformSchemaFailure ? "PLATFORM_SCHEMA" : repairBudgetExhausted ? "AUTO_REPAIR_EXHAUSTED" : browserTimeout ? "BROWSER_TIMEOUT" : source.name === "AbortError" || /超时|没有响应/i.test(message) || networkCode === "UND_ERR_CONNECT_TIMEOUT" ? "TIMEOUT"
  ...
  const safeMessage = code === "INVALID_INPUT" ? `修改意见未通过输入检查（${schemaPath(source) || "idea"}）：${String(inputIssue?.message ?? "").slice(0, 200) || "请补充说明。"}` : code === "PLATFORM_SCHEMA" ? `成品已通过全部验收，但冻结版本时未通过平台自身的数据格式校验（${schemaPath(source) || "未知字段"}），属于平台缺陷而非模型返回问题。` : code === "AUTO_REPAIR_EXHAUSTED" ? "平台已用完本轮有界修复预算，但当前产物仍未通过验收。" : code === "BROWSER_TIMEOUT" ? "真实浏览器验收超过总等待期限，已停止并回收浏览器资源。" : code === "TIMEOUT" ? "服务在等待期限内没有响应。" : code === "NETWORK" ? "无法连接服务。" : code === "AUTHENTICATION" ? "认证未通过。" : code === "PERMISSION" ? "当前模型或项目没有执行权限。" : code === "QUOTA" ? "服务额度不足。" : code === "RATE_LIMIT" ? "服务暂时限制了请求频率。" : code === "CONFIGURATION" ? "服务配置不完整或不兼容。" : code === "INVALID_RESPONSE" ? invalidImageResponse ? "服务未返回可解码的 PNG 图像数据。" : "服务返回内容格式无效，无法继续制作。" : code === "ASSET_INCOMPLETE" ? message : code === "INVALID_IMAGE" && /没有真实透明/.test(message) ? "返回图片缺少所需的透明背景。" : code === "INVALID_IMAGE" && size ? `返回图片尺寸 ${size[1]}×${size[2]}，无法满足目标 ${size[3]}×${size[4]}。` : code === "INVALID_IMAGE" && /比例/.test(message) ? "返回图片比例与目标不符，裁切后不足以保留要求的画面。" : code === "INVALID_IMAGE" ? "返回图片未通过本地 PNG 或尺寸校验。" : code === "VALIDATION" || code === "BROWSER_VALIDATION" ? "生成结果没有通过验收。" : http ? `服务返回 HTTP ${http[1]}。` : "构建遇到未分类错误，详细原因未安全记录。";
  const nextStep = code === "INVALID_INPUT" ? "请把修改意见写得更完整后重新提交；本次没有调用模型，也没有产生费用。" : code === "PLATFORM_SCHEMA" ? "本次产物与验收记录已保留；平台修复后可从检查点继续，不需要重新生成图片。" : code === "AUTO_REPAIR_EXHAUSTED" ? "当前代码、失败签名和复验记录均已保留；这是平台仍未解决的问题，不需要你修改代码、素材或验收标准。" : code === "AUTHENTICATION" ? "检查当前 API Key 与服务地址后，再由你明确重新制作。" : code === "PERMISSION" ? "确认该 Key 对所选模型和项目有权限后，再由你明确重新制作。" : code === "QUOTA" ? "补充服务额度或账单后，再由你明确重新制作。" : code === "RATE_LIMIT" ? "系统会在服务恢复后重新提交。" : code === "INVALID_IMAGE" ? "系统已用完本轮资源修复次数；下一轮将继续只重新生成并检查该资源。" : code === "CONFIGURATION" ? "检查图像或文本服务配置后，再由你明确重新制作。" : retryable ? "系统会在服务或网络恢复后重新提交。" : "系统将保留失败记录并继续从安全检查点恢复。";
