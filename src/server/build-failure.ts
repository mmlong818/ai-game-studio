...
export function safeFailure(stage: FailureDetail["stage"], error: unknown, extra: Partial<FailureDetail> = {}): FailureDetail {
  ...
  const code = platformSchemaFailure ? "PLATFORM_SCHEMA" : browserTimeout ? "BROWSER_TIMEOUT" : source.name === "AbortError" || /超时|没有响应/i.test(message) || networkCode === "UND_ERR_CONNECT_TIMEOUT" ? "TIMEOUT"
