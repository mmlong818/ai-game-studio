...
export function gameContentSecurityPolicy(frameAncestors = "'self'", scriptNonce?: string) {
  const scripts = scriptNonce ? `'self' 'nonce-${scriptNonce}'` : "'self'";
  return `default-src 'self'; img-src 'self' data: blob:; media-src 'self'; style-src 'self' 'unsafe-inline'; script-src ${scripts}; worker-src 'self' blob:; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors ${frameAncestors}`;
}
