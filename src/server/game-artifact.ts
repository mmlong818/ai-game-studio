...
document.querySelectorAll("[data-control]").forEach((button) => button.addEventListener("click", () => {
  if (gameSessionState === "playing") handleControl(button.dataset.control);
}));
window.addEventListener("keydown", (event) => {
  if (["INPUT", "SELECT", "TEXTAREA", "BUTTON", "SUMMARY", "A"].includes(event.target?.tagName)) return;
  if (gameSessionState !== "playing") return;
...
function templateHtml(project: ProjectDetail) {
  ...
  const snakeDifficultyOptions = [
    { id: "relaxed", label: "轻松", short: "慢速 · 8 枚", detail: "慢速巡游 · 8 枚朱果 · 越界会从另一侧回来" },
    { id: "standard", label: "标准", short: "适中 · 12 枚", detail: "适中速度 · 12 枚朱果 · 边界与身体都会中断巡游" },
    { id: "challenging", label: "挑战", short: "加速 · 18 枚", detail: "高速开局 · 18 枚朱果 · 收集后还会继续加速" },
  ];
