...
const COMPOSED_MECHANIC_SCENARIOS: Record<string, GoldenScenarioDefinition> = {
  ...
  "grid-slide-merge": { actions: { inspect: ["board-readable"], slide: ["board-slid", "equal-merged-once"], finish: ["target-tile-reached", "session-completed"] }, completingActions: ["finish"] },
  "projectile-wave": { actions: { aim: ["aim-readable"], fire: ["projectile-fired", "stable-hit"], finish: ["enemy-wave-settled", "session-completed"] }, completingActions: ["finish"] },
  "grid-navigation": { actions: { inspect: ["entry-exit-connected"], "invalid-wall": ["wall-blocked"], finish: ["exit-reached", "session-completed"] }, rejectedActions: ["invalid-wall"], completingActions: ["finish"] },
  "route-relic-synergy": { actions: { compare: ["risk-reward-visible"], choose: ["route-choice-applied"], finish: ["route-result-resolved", "session-completed"] }, completingActions: ["finish"] },
  "collect-charge": { actions: { collect: ["target-collected", "exit-unlocked"], retreat: ["risk-increased"], finish: ["exit-reached", "session-completed"] }, completingActions: ["finish"] },
  "drag-snap-assembly": { actions: { pick: ["piece-picked"], "invalid-drop": ["invalid-drop-rejected"], finish: ["piece-snapped", "session-completed"] }, rejectedActions: ["invalid-drop"], completingActions: ["finish"] },
  ...
  "sort-and-serve": { actions: { inspect: ["orders-and-deadlines-visible"], schedule: ["queue-order-applied", "capacity-respected"], finish: ["orders-delivered", "round-settled", "session-completed"] }, completingActions: ["finish"] },
  "choice-consequence": { actions: { read: ["chapter-state-visible"], choose: ["branch-choice-applied", "consequence-flag-saved"], finish: ["chapter-result-reached", "checkpoint-saved", "session-completed"] }, completingActions: ["finish"] },
  "deck-synergy": { actions: { draw: ["seeded-card-drawn"], play: ["card-cost-paid", "combo-resolved"], finish: ["turn-settled", "session-completed"] }, completingActions: ["finish"] },
  "gamepad-equivalent-control": { actions: { connect: ["gamepad-detected"], act: ["gamepad-action-mapped", "focus-feedback-visible"], finish: ["keyboard-fallback-preserved", "session-completed"] }, completingActions: ["finish"] },
  "spatial-rotation-path": { actions: { observe: ["camera-rotated", "spatial-clue-visible"], operate: ["mechanism-state-changed", "path-remains-reachable"], finish: ["spatial-goal-reached", "checkpoint-saved", "session-completed"] }, completingActions: ["finish"] },
};
...
export function createProbe(spec: GameSpecV2): GameProbe {
  ...
  if (
    ...
    spec.source.selectedMechanicIds.includes("collect-charge")
  ) {
