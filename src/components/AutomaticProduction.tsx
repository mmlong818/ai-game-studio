...
import { submitProduction, getProductionJob, watchProductionJob, getProject, getLatestBuild, retryProduction, startBuild, type ProductionJob } from "../web/api";
...
export function AutomaticProduction({ draft, confirmedPlan, confirmedDesignProfile, originalIdea }: { draft: StudioDraft; confirmedPlan?: string; confirmedDesignProfile?: GameDesignProfile; originalIdea?: string }) {
  ...
  const liveState = failed ? "failed" : succeeded ? "succeeded" : working || busy ? "running" : preparing ? "queued" : "idle";
  const liveStateLabel = failed ? "未完成" : succeeded ? "可试玩" : working || busy ? "制作中" : preparing ? "准备中" : "等待";
  const placeholderTitle = failed ? "本次制作未完成" : working ? "游戏正在这里成形" : preparing || busy ? "正在准备制作" : loaded ? "等待制作任务" : "正在恢复制作记录";
  const placeholderDetail = failed ? "已有项目记录保留，右侧步骤里有停下的位置和原因，可以直接重新制作。" : working ? (step?.detail ?? "服务端正在处理，关闭页面也不会取消任务。") : preparing ? (job?.events?.at(-1)?.title ?? "服务端已接收，等待开始处理") : "制作完成后，游戏会直接出现在这里。";
